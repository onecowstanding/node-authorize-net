  ...
  return P.resolve()
    ...
    .spread(createJsonCallback(function (json) {
      if (json.createTransactionResponse) {
        ...
        if (json.createTransactionResponse.messages &&
            json.createTransactionResponse.messages[0] &&
            json.createTransactionResponse.messages[0].resultCode &&
            json.createTransactionResponse.messages[0].resultCode[0] &&
            json.createTransactionResponse.messages[0].resultCode[0] === 'Error') {
          if (json.createTransactionResponse.transactionResponse &&
              json.createTransactionResponse.transactionResponse[0] &&
              json.createTransactionResponse.transactionResponse[0].errors &&
              json.createTransactionResponse.transactionResponse[0].errors[0] &&
              json.createTransactionResponse.transactionResponse[0].errors[0].error &&
              json.createTransactionResponse.transactionResponse[0].errors[0].error[0] &&
              json.createTransactionResponse.transactionResponse[0].errors[0].error[0].errorText &&
              json.createTransactionResponse.transactionResponse[0].errors[0].error[0].errorText[0]) {
            ...
            throw new GatewayError('Unable to parse payment gateway error response', json);
          }
        }
        return transactionCb(json.createTransactionResponse.transactionResponse[0])

      } else {
        throw new GatewayError('Unable to parse payment gateway response', json);
      }
    }))
};
...
AuthorizeNetGateway.prototype.submitTransaction = function submitTransaction (order, creditCard, prospect, other) {
  ...
  if (order.invoiceNumber || order.description) {
    body.order = { invoiceNumber: order.invoiceNumber || '', description: order.description || '' };
  }

  if (prospect) {
    if (prospect.billingEmailAddress) {
      body.customer = { email: prospect.billingEmailAddress };
    }
  ...
  return P.resolve()
    .then(function () {
      ...
      var body = {
        ...
        paymentProfiles: {
          billTo: mapKeys(billing, billToSchema),
          payment: {
            creditCard: {
...
AuthorizeNetGateway.prototype.chargeCustomer = function (order, prospect, other) {
  ...
  return this.getCustomerProfile(prospect.profileId)
    .then(function (res) {
      ...
      if (prospect.cvv2) {
        body.profile.paymentProfile.cardCode = prospect.cvv2;
      }
...
AuthorizeNetGateway.prototype.deleteCustomerProfile = function getCustomerProfile (profileId) {

  var service = this;
  return P.resolve()
    .then(function () {
      return  setRequest(service, 'deleteCustomerProfileRequest', {key: 'customerProfileId', value: profileId});
    })
    .then(sendXmlifiedRequest(service))
    .spread(createJsonCallback(function (json) {
      if (json.deleteCustomerProfileResponse) {

        var errors;

        if (json.deleteCustomerProfileResponse.messages[0].resultCode[0] === 'Error') {
          errors = json.deleteCustomerProfileResponse.messages[0].message[0];
          errors = errors.length > 1 ? errors : [errors];
          throw new GatewayError(errors.reduce(function (previous, current) {
            previous += '- ' + current.text;
            return previous;
          }, ''), json.deleteCustomerProfileResponse);
        }

        return {
          _original: json.deleteCustomerProfileResponse
        };
